...
//    public function sendMsg( $msg , $conn )
//    {
//    	$conn->
//    }
    ...
    public function setName( $name ,  $connection)
    {
    	if( $this->ifNameExist($name) ) 
    	{
    		$connection->send( 'login_该用户已经注册，请换一个用户名试试！' );
    		return ;
    	}else 
    	{
    		$connection->setUserName( $name );
    		$this->send('userAdd_'.$name) ;
    	}
    }
    public function updateState( $state , $connection )
    {
    	$this->send('userState_'.$connection->name.":::".$state ) ;
    }
    
    public function forwardData( $name , $msg )
    {
    	$conn = $this->getConnByName($name) ;
        if( $conn!=null )
    	{
    		$conn->send( $msg );
    	}
    ...
    public function getConnByName( $name )
    {
    	foreach ( $this->_clients as $v )
    	{
    		if( $v->name==$name )
    ...
    protected function ifNameExist($name)
    {
        foreach ($this->_clients as $v) 
        {
           //如果 该用户名已经注册
