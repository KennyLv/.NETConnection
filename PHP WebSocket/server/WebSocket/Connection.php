...
class Connection
{
    ...
    private function _handshake($data)
    {
        ...
        if (array_key_exists('Sec-WebSocket-Key', $headers) && $headers['Sec-WebSocket-Version'] >= 6) {
			...
			//先解析客户端（浏览器）发来的header字符串，将内存存放到关联数组$headers中(此处代码省略)
    ...
    public function onDisconnect()
    {
        ...
        //通知 所有用户  
        $this->_server->send( "exit_".$this->name );
    ...
    public function setName( $name )
    {
    	if( $name!='' )
    	{
    		$this->_server->setName( $name , $this ) ;
    	}
    }
    public function updateState( $state )
    {
    	...
    	{
    		$this->state = $state ;
    		$this->_server->updateState( $state , $this );
    	}
    ...
    {
    	$this->_server->forwardData( $name , $msg ) ;
    }
    
    public function setUserName( $name )
    {
    	$this->name = $name ;
    	$this->send('login_Accept') ;
    }
    
    public function logout()
    {
    	$this->_server->send( 'logout_'.$this->name );
    	$this->name = '' ;
    }
    
    public function getUsers()
    {
    	$this->_server->getUsers($this);
    }
